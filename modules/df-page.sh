...
for timeFrame in $timeFrameList;do
	# set the webpage url
	webpageUrl="/var/cache/hackbox-system-monitor/df-$timeFrame.html";
	# touch the webpage
	touch $webpageUrl;
	...
	echo "<a href='df-day.html'><span>Daily</span></a>" >> $webpageUrl;
	echo "<a href='df-week.html'><span>Weekly</span></a>" >> $webpageUrl;
	echo "<a href='df-month.html'><span>Monthly</span></a>" >> $webpageUrl;
	echo "<a href='df-year.html'><span>Yearly</span></a>" >> $webpageUrl;
	# create link to munin directory
	echo "<a href='www/'><span>Munin</span></a>" >> $webpageUrl;
	echo "</div>" >> $webpageUrl;
	echo "<br /><hr />" >> $webpageUrl;
	tempFileName=df-$timeFrame.png
	...
	echo "<h1>Filesystem Usage</h1>" >> $webpageUrl;
	echo "<hr />" >> $webpageUrl;
	# set preformated text for the printer output
	echo "<pre>" >> $webpageUrl;
	# print all the previous and existing printer jobs
	df -h | grep -v tmpfs | grep -v cgmfs | grep -v udev >> $webpageUrl
	echo "</pre>" >> $webpageUrl
