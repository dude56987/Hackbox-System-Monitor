...
for timeFrame in $timeFrameList;do
	# set the webpage url
	webpageUrl="/var/cache/hackbox-system-monitor/memory-$timeFrame.html";
	# touch the webpage
	touch $webpageUrl;
	...
	echo "<a href='memory-day.html'><span>Daily</span></a>" >> $webpageUrl;
	echo "<a href='memory-week.html'><span>Weekly</span></a>" >> $webpageUrl;
	echo "<a href='memory-month.html'><span>Monthly</span></a>" >> $webpageUrl;
	echo "<a href='memory-year.html'><span>Yearly</span></a>" >> $webpageUrl;
	# create link to munin directory
	echo "<a href='www/'><span>Munin</span></a>" >> $webpageUrl;
	echo "</div>" >> $webpageUrl;
	echo "<br /><hr />" >> $webpageUrl;
	tempFileName=memory-$timeFrame.png
	...
	echo "<h1>Memory Stats</h1>" >> $webpageUrl;
	echo "<hr />" >> $webpageUrl;
	echo "<pre>" >> $webpageUrl;
	# print memory info
	free -ht >> $webpageUrl
